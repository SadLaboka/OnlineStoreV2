...
class CategoryManager(models.Manager):
    ...
    def get_categories_for_sidebar(self):
        ...
        data = [
            dict(title=c.title, url=c.get_absolute_url(), count=getattr(c, self.CATEGORY_TITLE_COUNT_NAME[c.title]))
            for c in qs
        ]
        return data
...
class CartProduct(models.Model):
    ...
    final_price = models.DecimalField(max_digits=9, decimal_places=2, verbose_name='Суммарная цена')
...
class Customer(models.Model):
    ...
    address = models.CharField(max_length=255, verbose_name='Адерс')
